...
export default function createStore(initial_state: StoreState): Store {
	...
	function getTreeMainDatum(): TreeDatum {
		...
		const found = state.tree.data.find(
			(d) => d.data.id === state.main_id,
		);
	...
	function updateMainId(id: Datum['id']) {
		...
		state.main_id_history = state
			.main_id_history!.filter((d) => d !== id)
			.slice(-10);
	...
	function validateMainId() {
		if (state.main_id) {
			const mainExists = state.data.find(
				(d) => d.id === state.main_id,
			);
	...
	function checkIfFmFormat(data: LegacyDatum[]) {
		...
		for (let d of data) {
			if (d.rels.parent1 || d.rels.parent2) {
