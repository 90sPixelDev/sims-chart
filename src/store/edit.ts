...
export function syncRelReference(datum: Datum, data_stash: Data) {
	Object.keys(datum.data).forEach((k) => {
		if (k.includes('__ref__')) {
			...
			if (!rel) return;
			const ref_field_id =
				k.split('__ref__')[0] + '__ref__' + datum.id;
	...
	function executeDelete() {
		data_stash.forEach((d) => {
			for (let k in d.rels) {
				...
				if (
					Array.isArray(d.rels[key]) &&
					d.rels[key].includes(datum.id)
				) {
					d.rels[key].splice(
		...
		if (data_stash.length === 0)
			data_stash.push(createNewPerson({ data: { gender: 'U' } }));
	}
